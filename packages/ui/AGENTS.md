...
- Use token-driven visual utilities where relevant: `gradient-primary`, `gradient-primary/50`, `border-animation-primary`, `border-animation-primary/50`, `surface="flat" | "gradient" | "bare"` and glass effect controlled by `data-glass-effect`. Do not reintroduce `primary|contrast` border or gradient props.
